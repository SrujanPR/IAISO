...
import { ArrowRight, Network, Database, GitBranch } from 'lucide-react';
...
const IntegrateSection = () => {
  ...
  return (
    <section
      ref={sectionRef}
      id="integrate"
      className="relative w-full h-screen bg-navy-900 overflow-hidden z-20"
    >
      {/* Background video */}
      <video
      ...
      <svg
        ref={ringsRef}
        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] pointer-events-none opacity-0"
        viewBox="0 0 700 700"
      >
        <circle
          cx="350"
      ...
      <div className="relative z-10 w-full h-full flex items-center px-6 lg:px-[10vw]">
        ...
        <div className="relative z-10 flex-1 max-w-xl">
          ...
          >
            Unify data, APIs, and infrastructure with clean architecture and modern DevOps.
          </p>

          {/* CTA */}
          <button
            ref={ctaRef}
            className="group flex items-center gap-2 px-6 py-3 bg-secondary border border-rooman-orange text-rooman-orange font-medium rounded-lg hover:bg-rooman-orange hover:text-white transition-all duration-300 hover:-translate-y-0.5"
          >
        ...
        >
          <FeatureChip icon={Network} label="API Gateway" delay={0} />
          <FeatureChip icon={Database} label="Data Pipelines" delay={100} />
          <FeatureChip icon={GitBranch} label="DevOps" delay={200} />
        </div>
      </div>
